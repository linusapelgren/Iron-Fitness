...
from datetime import datetime, time, timedelta  # Import datetime, time, timedelta for time manipulation
...
def generate_time_choices(start_hour=8, end_hour=16, interval_minutes=60):
    ...
    while current_time < end_time:
        end_time_option = (datetime.combine(datetime.today(), current_time) + timedelta(minutes=interval_minutes)).time()
        times.append((f"{current_time.strftime('%H:%M')}-{end_time_option.strftime('%H:%M')}", f"{current_time.strftime('%H:%M')}-{end_time_option.strftime('%H:%M')}"))
...
class BookingForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['fitness_class'].choices = [(choice[0], choice[1]) for choice in ClassTime._meta.get_field('fitness_class').choices]
        self.initial.update(initial_data)

        # Get class and day from data or initial
        fitness_class = self.data.get('fitness_class') or initial_data.get('fitness_class')
        ...
        if fitness_class and class_day:
            self.fields['class_time'].choices = self.get_class_times_for_day_and_class(fitness_class, class_day)
        else:

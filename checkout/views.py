...
from django.views.decorators.csrf import csrf_exempt  # Import to exempt views from CSRF verification
from django.shortcuts import render, get_object_or_404  # Import render to render templates and get_object_or_404 for querying models
from django.contrib.auth.decorators import login_required  # Import login_required to restrict access to logged-in users
...
def process_payment(request):
    if request.method == 'POST':
        ...
        try:
            plan = get_object_or_404(SubscriptionPlan, pk=plan_id)  # Get the subscription plan or return 404
            ...
            return JsonResponse({'error': str(e)}, status=500)  # Return an error response in case of a Stripe error

    return JsonResponse({'error': 'Invalid request'}, status=400)  # Return an error response for invalid requests

# Define a view for the checkout process
@login_required
def checkout(request, plan_id):
    plan = get_object_or_404(SubscriptionPlan, pk=plan_id)  # Get the subscription plan or return 404
    user_profile = UserProfile.objects.get(user=request.user) if request.user.is_authenticated else None  # Get the user profile if authenticated
    ...
    return render(request, 'checkout/checkout.html', context)  # Render the checkout template with context

# Define a view to provide an overview of an order
def order_overview(request, plan_id):
    try:
        plan = SubscriptionPlan.objects.get(pk=plan_id)  # Get the subscription plan or raise an exception
        data = {
            'name': plan.name,
        ...
        return JsonResponse({'error': 'Plan not found'}, status=404)  # Return an error response if the plan does not exist
...
def stripe_webhook(request):
    ...
    sig_header = request.META.get('HTTP_STRIPE_SIGNATURE', '')  # Get the Stripe signature header
    endpoint_secret = settings.STRIPE_WEBHOOK_SECRET  # Get the webhook secret from settings

    try:
        event = stripe.Webhook.construct_event(payload, sig_header, endpoint_secret)  # Construct the event from the payload and signature
    ...
    if event['type'] == 'checkout.session.completed':  # Check if the event is for a completed checkout session
...
def handle_checkout_session(session):
    plan_id = session.get('metadata', {}).get('plan_id')  # Get the plan ID from the session metadata
    email = session.get('metadata', {}).get('email')  # Get the email from the session metadata

    if not plan_id or not email:
        logger.error("Missing plan_id or email in session metadata.")  # Log an error if metadata is missing
        return

    try:
        user = User.objects.get(email=email)  # Get the user by email
        plan = SubscriptionPlan.objects.get(id=plan_id)  # Get the subscription plan by ID
        ...
        logger.info(f"Successfully updated profile for {user.email} with plan {plan.name}")  # Log a success message

    except User.DoesNotExist:
        logger.error(f"User with email {email} does not exist.")  # Log an error if the user does not exist
    except SubscriptionPlan.DoesNotExist:
        logger.error(f"Subscription plan with ID {plan_id} does not exist.")  # Log an error if the plan does not exist
